...
import { verifyAndDepositFunds } from "./actions";
import { calculateGrossAmount } from "@/utils/paystack";
import { PaystackButton } from "react-paystack";

export default function WalletPage({ wallet, transactions, email }: { wallet: any, transactions: any[], email: string }) {
    ...
    const paystackConfig = {
        ...
        publicKey: publicKey,
        text: isDepositing ? "Processing..." : "Deposit Funds",
        onSuccess: async (reference: any) => {
            setIsDepositing(true);
            const result = await verifyAndDepositFunds(reference.reference, netAmount);
            if (result.error) {
                toast.error(result.error);
            } else {
                toast.success(`Successfully deposited ₦${netAmount.toLocaleString()}!`);
                setAmount("5000");
            }
    ...
    const isValidAmount = !isNaN(netAmount) && netAmount > 0;
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div id="deposit-section" className="bg-indigo-50/50 rounded-3xl p-8 mb-10 border border-indigo-100 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-md">
                    ...
                    <p className="text-sm text-[#3A5369]/70 mb-6">Instantly and securely add funds to your wallet using Paystack.</p>
                    ...
                    {grossAmount > netAmount && netAmount > 0 && (
                        <p className="text-xs text-orange-600 font-medium mb-3 bg-orange-50 p-2 rounded border border-orange-100">
                            A small transaction fee will be added. You will be charged exactly ₦{grossAmount.toLocaleString()} to receive ₦{netAmount.toLocaleString()}.
                        </p>
                    )}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isValidAmount && publicKey ? (
                            ...
                            <button
                                disabled
                                className="bg-[#1A1A2E] opacity-40 cursor-not-allowed text-white px-6 py-3 rounded-xl font-bold text-sm flex items-center gap-2"
                            >
                            ...
                            {transactions.map(tx => (
                                <div key={tx.id} className="p-6 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <div className="text-right">
                                        <p className={`font-bold ${tx.type === 'deposit' ? 'text-green-600' : 'text-[#1A1A2E]'}`}>
